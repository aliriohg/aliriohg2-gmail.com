...
aws s3 cp s3://${REPORT_BUCKET}/logs/ target/gatling/logs --recursive --profile ${AWS_DEFAULT_PROFILE}
...
aws s3 cp target/gatling/ s3://${REPORT_BUCKET}/gatling/$FOLDER --recursive --profile ${AWS_DEFAULT_PROFILE}